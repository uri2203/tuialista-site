...
(function (global) {
  ...
  /* ══════════════════════════════════════════════════════════════════════
     FICHAS DETALLADAS POR AGENTE (retrieval) — datos REALES de los manuales.
     Se incluye SOLO la ficha del/los agente(s) que menciona el usuario, para
     no inflar el prompt (16 manuales completos no caben). Precios = catálogo real.
     buildAgentDetail() arma el texto; matchAgents() elige por palabra clave.
     ══════════════════════════════════════════════════════════════════════ */
  var agentDetails = [
  ...
  function matchAgents(message) {
    var q = normalize(message || "");
    if (!q) return [];
    var scored = [];
    for (var i = 0; i < agentDetails.length; i++) {
      var a = agentDetails[i], s = 0;
      for (var k = 0; k < a.kw.length; k++) {
        var w = normalize(a.kw[k]);
        if (w && q.indexOf(w) >= 0) s += w.indexOf(" ") >= 0 ? 2 : 1;
      }
      if (s > 0) scored.push({ a: a, s: s });
    }
    scored.sort(function (x, y) { return y.s - x.s; });
    return scored.slice(0, 2).map(function (o) { return o.a; });
  }
  ...
  var ui = {
    es: {
      ...
      humanLink: "¿No te ayudo? Habla con un humano",
      humanTitle: "Escríbenos y te contactamos",
      humanName: "Tu nombre",
      humanEmail: "Tu correo",
      humanMessage: "¿En qué te ayudamos?",
      humanSubmit: "Enviar",
      humanCancel: "Cancelar",
      humanSuccess: "¡Listo! Te contactaremos por correo.",
      humanError: "No se pudo enviar. Escríbenos a soporte@tuialista.com.",
    },
    en: {
      ...
      close: "Close",
      open: "Open assistant",
      humanLink: "Not helping? Talk to a human",
      humanTitle: "Write to us and we'll reach out",
      humanName: "Your name",
      humanEmail: "Your email",
      humanMessage: "How can we help?",
      humanSubmit: "Send",
      humanCancel: "Cancel",
      humanSuccess: "Done! We'll reach out by email.",
      humanError: "Couldn't send it. Write to us at soporte@tuialista.com.",
    },
    pt: {
      ...
      humanLink: "Não ajudei? Fale com um humano",
      humanTitle: "Escreva-nos e entraremos em contato",
      humanName: "Seu nome",
      humanEmail: "Seu e-mail",
      humanMessage: "Como podemos ajudar?",
      humanSubmit: "Enviar",
      humanCancel: "Cancelar",
      humanSuccess: "Pronto! Entraremos em contato por e-mail.",
      humanError: "Não foi possível enviar. Escreva para soporte@tuialista.com.",
    },
    fr: {
      ...
      humanLink: "Ça ne t'aide pas ? Parle à un humain",
      humanTitle: "Écris-nous, on te recontacte",
      humanName: "Ton nom",
      humanEmail: "Ton e-mail",
      humanMessage: "Comment pouvons-nous aider ?",
      humanSubmit: "Envoyer",
      humanCancel: "Annuler",
      humanSuccess: "C'est fait ! On te recontacte par e-mail.",
      humanError: "Échec de l'envoi. Écris-nous à soporte@tuialista.com.",
    },
    de: {
      ...
      humanLink: "Hilft nicht? Mit Mensch sprechen",
      humanTitle: "Schreib uns, wir melden uns",
      humanName: "Dein Name",
      humanEmail: "Deine E-Mail",
      humanMessage: "Wie können wir helfen?",
      humanSubmit: "Senden",
      humanCancel: "Abbrechen",
      humanSuccess: "Fertig! Wir melden uns per E-Mail.",
      humanError: "Senden fehlgeschlagen. Schreib an soporte@tuialista.com.",
    },
    it: {
      ...
      humanLink: "Non ti aiuto? Parla con un umano",
      humanTitle: "Scrivici e ti contatteremo",
      humanName: "Il tuo nome",
      humanEmail: "La tua e-mail",
      humanMessage: "Come possiamo aiutarti?",
      humanSubmit: "Invia",
      humanCancel: "Annulla",
      humanSuccess: "Fatto! Ti contatteremo via e-mail.",
      humanError: "Invio non riuscito. Scrivici a soporte@tuialista.com.",
    },
  };
  ...
  function fullSystemPrompt(mode, userMessage) {
    ...
    if (mode !== "affiliate" && userMessage) {
      var hits = matchAgents(userMessage);
      for (var i = 0; i < hits.length; i++) {
        out += "\n\n## " + buildAgentDetail(hits[i]);
      }
    }
    return out;
  }
  ...
  global.TUIA_KB = {
    ...
    matchAgents: matchAgents,
    buildAgentDetail: buildAgentDetail,
